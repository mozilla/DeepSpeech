...
std::vector<std::string> Scorer::make_ngram(PathTrie* prefix)
{
  ...
  for (int order = 0; order < max_order_; order++) {
    ...
    if (is_utf8_mode_) {
      new_node = current_node->get_prev_grapheme(prefix_vec, prefix_steps, prefix_probabilities);
    } else {
      new_node = current_node->get_prev_word(prefix_vec, prefix_steps, prefix_probabilities, SPACE_ID_);
    }
