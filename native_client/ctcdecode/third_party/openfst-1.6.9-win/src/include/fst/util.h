...
bool ReadIntPairs(const string &filename, std::vector<std::pair<I, I>> *pairs,
  ...
  while (strm.getline(line, kLineLen)) {
    ...
    bool err;
    I i1 = StrToint64_t(col[0], filename, nline, allow_negative, &err);
    if (err) return false;
    I i2 = StrToint64_t(col[1], filename, nline, allow_negative, &err);
    if (err) return false;
...
bool WriteIntPairs(const string &filename,
  ...
  for (std::ptrdiff_t n = 0; n < pairs.size(); ++n) {
