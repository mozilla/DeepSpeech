...
package_native_client()
{
  ...
  ${TAR} -cf - \
    -C ${tensorflow_dir}/bazel-bin/native_client/ libaeiou.so \
    -C ${tensorflow_dir}/bazel-bin/native_client/ libaeiou.so.if.lib \
    -C ${tensorflow_dir}/bazel-bin/native_client/ generate_scorer_package \
    -C ${deepspeech_dir}/ LICENSE \
    -C ${deepspeech_dir}/native_client/ aeiou${PLATFORM_EXE_SUFFIX} \
    -C ${deepspeech_dir}/native_client/ aeiou.h \
...
package_native_client_ndk()
{
  ...
  tar -cf - \
    -C ${deepspeech_dir}/native_client/libs/${arch_abi}/ aeiou \
    -C ${deepspeech_dir}/native_client/libs/${arch_abi}/ libaeiou.so \
    -C ${tensorflow_dir}/bazel-bin/native_client/ generate_scorer_package \
...
package_libdeepspeech_as_zip()
{
  ...
  zip -r9 --junk-paths "${artifacts_dir}/${artifact_name}" ${tensorflow_dir}/bazel-bin/native_client/libaeiou.so
}
