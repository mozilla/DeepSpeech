...
unset PYTHONPATH
export PYENV_ROOT="${HOME}/ds-transfer/.pyenv"
export PATH="${PYENV_ROOT}/bin:${HOME}/bin:$PATH"

mkdir -p ${PYENV_ROOT} || true
mkdir -p ${TASKCLUSTER_ARTIFACTS} || true
mkdir -p /tmp/transfer || true
mkdir -p /tmp/transfer_tflite || true

ckpt_source="${TASKCLUSTER_TMP_DIR}/ckpt_source/"
mkdir -p ${ckpt_source}
download_checkpoint "https://github.com/mozilla/DeepSpeech/releases/download/v0.6.1/deepspeech-0.6.1-checkpoint.tar.gz" "${ckpt_source}"

install_pyenv "${PYENV_ROOT}"
install_pyenv_virtualenv "$(pyenv root)/plugins/pyenv-virtualenv"

PYENV_NAME=deepspeech-transfer
...
pushd ${HOME}/DeepSpeech/ds/
    ...
    time ./bin/run-tc-ru_transfer.sh 5 "${sample_rate}"
popd
