...
def _frame_generator(audio, starttime, stoptime, sample_rate):
    timestamp = starttime
    n = int(sample_rate * (frame_duration_ms / 1000.0) * 2)
    duration = (float(n) / sample_rate) / 2.0
    offset = int(starttime / duration)
    stop_offset = int(stoptime / duration)
    assert stop_offset <= len(audio)
    while offset + n < stop_offset:
...
def _find_vocalization_start(audiofile, starttime, stoptime):
  ...
  audio, sample_rate = _read_wave(audiofile)
  frames = _frame_generator(frame_duration_ms, audio, starttime, stoptime, sample_rate)
  frames = list(frames)
  for frame in frames:
    if vad.is_speech(frame.bytes, sample_rate):
      vocalization_start = frame.timestamp
      break
...
def _find_vocalization_stop(audiofile, starttime, stoptime):
  ...
  for frame in frames:
    if not vad.is_speech(frame.bytes, sample_rate):
      vocalization_stop = frame.timestamp
      break
...
def _process_xml(dataset, search_dir, data_dir, name, data):
    ...
    for audiofile in _find_files(search_dir, ['wav', 'mp3', 'ogg', 'flac'], name):
        ...
        convertedfile = os.path.join(data_dir, os.path.splitext(os.path.basename(audiofile))[0] + '.wav')
        ...
        for i in range(len(snippets) - 2):
            ...
            naive_duration = (snippets[i + 1][0] - snippets[i][0])
            min_stoptime = max(0, (starttime + naive_duration - epsilon))
            max_stoptime = min(endtime, (starttime + naive_duration + epsilon))
