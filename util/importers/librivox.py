...
def read_data_sets(data_dir, batch_size, numcep, numcontext, thread_count=8, limits=(0, 0, 0), rotate=True):
    ...
    train = _read_data_set(work_dir, "train-*-wav", thread_count, batch_size, numcep, numcontext, limit=limit[2], rotate=rotate)

    # Create dev DataSet from all the dev archives
    dev = _read_data_set(work_dir, "dev-*-wav", thread_count, batch_size, numcep, numcontext, limit=limit[0], rotate=rotate)

    # Create test DataSet from all the test archives
    test = _read_data_set(work_dir, "test-*-wav", thread_count, batch_size, numcep, numcontext, limit=limit[1], rotate=rotate)
...
def _read_data_set(work_dir, data_set, thread_count, batch_size, numcep, numcontext, limit=0, rotate=True):
    # Create data set dir
