...
def BiRNN(batch_x, seq_length, dropout):
    ...
    lstm_fw_cell = tf.contrib.rnn.MultiRNNCell(cells=[_forward_cell() for _ in range(3)], state_is_tuple=True)
    ...
    lstm_bw_cell = tf.contrib.rnn.MultiRNNCell(cells=[_backward_cell() for _ in range(3)], state_is_tuple=True)
